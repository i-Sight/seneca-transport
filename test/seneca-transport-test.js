/* Copyright (c) 2014-present Richard Rodger, MIT License */
...
function foo_service(seneca, type, port) {
	return seneca
		...
		.listen({type, port: (port < 0 ? -1 * port : port)})
		.listen({
			type,
...
function foo_test(tname, require, fin, type, port) {
	...
	service.ready(function (){
		const client = foo_run(service, done, type, port);
...
function foo_pintest(tname, require, fin, type, port) {
	...
	service.ready(function (){
		const client = foo_pinrun(service, done, type, port);
...
function foo_fault(require, type, port, speed) {
	...
	service.ready(function (){
		let client = require('seneca')({log: 'silent'})
          	.use('..')
          	.client({type, port});
		...
		setTimeout(function (){
			...
			client = require('seneca')({log: 'silent'})
        	.use('..')
        	.client({type, port});
